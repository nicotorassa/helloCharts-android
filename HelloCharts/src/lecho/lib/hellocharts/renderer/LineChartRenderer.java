...
public class LineChartRenderer extends AbstractChartRenderer {
	...
	private int botColor, topColor;

	public LineChartRenderer(Context context, Chart chart,
			LineChartDataProvider dataProvider, int botColor, int topColor) {
		super(context, chart);
		this.botColor = botColor;
		this.topColor = topColor;
		this.dataProvider = dataProvider;

		touchToleranceMargin = ChartUtils.dp2px(density,
				DEFAULT_TOUCH_TOLERANCE_MARGIN_DP);
	...
	public void onChartSizeChanged() {
		...
		computator.insetContentRectByInternalMargins(internalMargin,
				internalMargin, internalMargin, internalMargin);
		if (computator.getChartWidth() > 0 && computator.getChartHeight() > 0) {
			softwareBitmap = Bitmap.createBitmap(computator.getChartWidth(),
	...
	public void draw(Canvas canvas) {
		...
		// softwareBitmap can be null if chart is rendered in layout editor. In
		// that case use default canvas and not
		// softwareCanvas.
	...
	private void calculateMaxViewport() {
		tempMaximumViewport.set(Float.MAX_VALUE, Float.MIN_VALUE,
	...
	/**
	 * Draws lines, uses path for drawing filled area on software canvas. Line
	 * is drawn with canvas.drawLines() method.
	 */
...
//nunca
	...
	private void drawLabel(Canvas canvas, Line line, PointValue pointValue,
		...
		final int numChars = line.getFormatter().formatChartValue(labelBuffer,
				pointValue);
		if (numChars == 0) {
	...
	private void drawArea(Canvas canvas, Line line) {
		...
		final Rect contentRect = computator.getContentRectMinusAllMargins();
		final float baseRawValue = Math.min(contentRect.bottom,
				Math.max(computator.computeRawY(baseValue), contentRect.top));
		// That checks works only if the last point is the right most one.
		final float left = Math.max(
				computator.computeRawX(line.getValues().get(0).getX()),
				contentRect.left);
		final float right = Math.min(
				computator.computeRawX(line.getValues().get(lineSize - 1)
						.getX()), contentRect.right);
